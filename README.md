...
A shared sequencing layer for multiple rollups, with optional external arbitration supporting neutral, SLA-focused governance. The core framework is rollup-agnostic, with an OP Stack PoC living in `src/poc/opstack` and `script/opstack`.

## What is ISOCHRON?

**ISOCHRON** centralizes sequencing authority for multiple rollups (including OP Stack) into a single "Hub" contract while preserving individual sovereignty through opt-in rotation and SLA-based governance. The active sequencer can use MEV-Boost + Flashblocks-style private orderflow as a default, while remaining upgradeable to future public or hybrid builders without hard-coding any trusted hardware.
...
## Universal Sequencing Roadmap

ISOCHRON is evolving into a universal shared sequencing layer that:
- Enforces **cross-chain bundle execution** for atomic, multi-domain transactions.
- Supports **interoperability + unified liquidity** across chains without altering member chain stacks.
- Offers **fairer MEV distribution** through configurable builder policies and sovereign chain rules.
- Uses **adapters** to integrate with OP Stack, other EVM L2s, and future ecosystems like Cosmos.
- Implements **time-sensitive components in Rust** for reliability and performance.

The current PoC focuses on OP Stack, but the architecture is designed to be chain-agnostic:
- **Adapters, not forks**: chain-specific integration happens in adapters, not in shared core contracts.
- **Builder flexibility**: MEV-Boost + Flashblocks is the default private mempool mode today, but the hub is designed to accept public or trustless builders later.
- **Chain sovereignty**: each chain can declare sequencing policies the universal sequencer must obey; deterministic fraud proofs are preferred, with optional external arbitration for subjective criteria.

## Policies & SLAs

ISOCHRON governance is anchored by formal policies enforced through external arbitration:
...
- **Governance**: SharedSequencerHub with mock arbitration registry
...
9. **SLA Enforcement**: Misbehaving proposers challenged via external arbitration
...
4. **Register Operators in the Arbitration Registry**

Visit your registry UI (Curate-compatible) and submit proposer entries as required by your registry schema.
...
**Prerequisites:**
- Curate-compatible TCR deployed with your sequencer policy
...
- Adapters must be registered in the **Adapter Registry** (Curate-compatible)
...
### SLA Compliance
- SLA requirements enforced via external arbitration
...
### Griefing Mitigation
- High deposit requirement in the arbitration system deters frivolous challenges
...
│       ├── ICurate.sol               # Curate registry interface
...
A: ISOCHRON enforces SLA expectations (liveness, authorized production, and clean handoffs) through the Sequencer Policy and external arbitration.
...
A: ISOCHRON is designed to eventually replace the Superchain Council multisig. Instead of a committee updating rollup configuration contracts, the SharedSequencerHub does it algorithmically based on governance decisions.
