...
A shared sequencing layer for multiple rollups, with configurable arbitration supporting neutral, SLA-focused governance (Kleros Court is the current default). The core framework is rollup-agnostic, with an OP Stack PoC living in `src/poc/opstack` and `script/opstack`.

## What is ISOCHRON?

**ISOCHRON** (formerly known as KSSN) centralizes sequencing authority for multiple rollups (including OP Stack) into a single "Hub" contract while preserving their individual sovereignty through opt-in rotation and SLA-based governance. The active sequencer can use Rollup Boost and Flashblocks to keep the mempool private while enabling market-driven MEV without enshrining proposer-builder separation.
...
ISOCHRON governance is anchored by formal policies enforced through configurable arbitration (with Kleros Court as the default arbitrator):
...
- **Governance**: SharedSequencerHub with a mock arbitrator-backed registry (Kleros Court is the default in production)
...
9. **SLA Enforcement**: Misbehaving proposers challenged via the default arbitrator (Kleros Court today)
...
4. **Register Operators in the Default Arbitrator (Kleros Court)**
...
**Prerequisites:**
- Curated TCR deployed with your sequencer policy (Kleros Curate by default)
...
### SLA Compliance
- SLA requirements enforced via the default arbitrator (Kleros Court today)
...
### Griefing Mitigation
- High deposit requirement in the default TCR (Kleros Curate today) deters frivolous challenges
...
│       ├── ICurate.sol               # Curated TCR interface (Kleros Curate default)
...
├── script/
│   ├── DeployKSSN.s.sol              # ISOCHRON Hub-and-Spoke deployment (script name retained)
...
├── agent/
│   ├── kssn_proposer_agent.py        # ISOCHRON proposer agent (filename retained)
│   ├── kssn_config.example.yaml      # ISOCHRON agent config template (filename retained)
...
**Q: What is ISOCHRON?**
A: ISOCHRON (Interconnected Sequencing Oracle for Cross-chain Harmonized Reliability, Ordering & Network) is a Hub-and-Spoke architecture that manages sequencing for multiple rollups from a single Hub contract. It enables atomic cross-chain composability while preserving chain sovereignty through opt-in rotation and SLA governance.
...
A: ISOCHRON enforces SLA expectations (liveness, authorized production, and clean handoffs) through the Sequencer Policy and configurable arbitration (Kleros Court by default).
...
A: ISOCHRON is designed to eventually replace the Superchain Council multisig. Instead of a committee updating rollup configuration contracts, the SharedSequencerHub does it algorithmically based on default arbitrator rulings (Kleros Court today).
