...
This document defines the acceptance policy for the **Adapter Registry**, a curated list (Kleros Curate by default) that governs which rollup adapters can be used by SharedSequencerHub to rotate sequencers.
...
1. **Default Arbitration (Kleros Court today)**: Adapter must be registered in the Adapter Registry
2. **Ratchet Versioning**: New adapter version must be strictly greater than current

## Registry Type

- **Registry**: Curated TCR (Kleros Curate by default)
...
- Contains upgrade mechanisms that bypass default arbitrator governance (Kleros Court today)
